...
async function getFilteredProducts(
  ...
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/Products?${params.toString()}`, {
    next: { revalidate: 60 },
  });
...
async function getCategories(): Promise<string[]> {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/Products/categories`, {
    next: { revalidate: 60 },
  });
  if (!res.ok) return [];
  return res.json();
